            ...
            {(values) => (
                <Form className={classes.fullWidth}>
                    <Grid spacing={1} justifyContent="flex-start" container>
                        ...
                        <Grid xs={12} sm={8} md={2} item>
                            <FormControl margin="dense" variant="outlined" fullWidth>
                                <InputLabel htmlFor="useKanban-selection">letrero</InputLabel>
                                <Field
                                    as={Select}
...
export default function PlansManager() {
    ...
    const loadPlans = async () => {
        ...
        try {
            ...
            toast.error('No fue posible llevar la lista de registros')
        }
        setLoading(false)
    }
    ...
    const handleSubmit = async (data) => {
        ...
        try {
            ...
            toast.error('No fue posible realizar la operación.Asegúrese de que ya haya un plan con el mismo nombre o si los campos se llenaron correctamente')
        }
        setLoading(false)
    }
