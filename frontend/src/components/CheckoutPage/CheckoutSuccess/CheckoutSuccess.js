  ...
  useEffect(() => {
    ...
    socket.on(`company-${companyId}-payment`, (data) => {
      ...
      if (data.action === "CONCLUIDA") {
        toast.success(`Su licencia fue renovada hasta ${dateToClient(data.company.dueDate)}!`);
        setTimeout(() => {
          history.push("/");
        }, 4000);
      }
  ...
  return (
    <React.Fragment>
      ...
      <SuccessContent>
        ...
        <span>
          Finalmente, simplemente haga el escaneo de pago o pegando el
Código de pix arriba :)
        </span>
      </SuccessContent>
    </React.Fragment>
  );
