  ...
  const handleResetPassword = async (values) => {
    ...
    if (newPassword === confirmPassword) {
      try {
        ...
        toast.success(i18n.t("Contraseña redefinida exitosa."));
        history.push("/login");
      } catch (err) {
        console.log(err);
      }
  ...
  const UserSchema = Yup.object().shape({
    ...
    newPassword: isResetPasswordButtonClicked
      ? Yup.string()
          ...
          .matches(
            passwordRegex,
            "Su contraseña debe tener al menos 8 caracteres, uno en mayúsculas, un minúscula y un número."
          )
      : Yup.string(), // Sem validação se não for redefinição de senha
    confirmPassword: Yup.string().when("newPassword", {
      is: (newPassword) => isResetPasswordButtonClicked && newPassword,
      then: Yup.string()
        .oneOf([Yup.ref("newPassword"), null], "Las contraseñas no corresponden")
        .required("Campo obligatorio"),
